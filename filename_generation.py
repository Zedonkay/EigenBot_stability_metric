...
def filename_clean(frequency = int, test=str, control_type=str):
    return "1_clean_data/"+control_type+"/"+str(frequency)+"Hz.csv"
def filename_raw_test(frequency=int,test=str,control_type=str):
    return "2_raw_data/official_tests/"+control_type+"odometry_data_"+control_type+"_test_"+str(frequency)+"hz_"+test_number+".csv"
...
def store_clean_data(frequency = int, test=str, control_type=str):
    return "6_results/Clean_data."+control_type+str(frequency)+test+"Hz_"
