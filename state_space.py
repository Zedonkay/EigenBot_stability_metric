...
def plot_2d(timestamps, pos_x, pos_y, pos_z, vel_x, vel_y, vel_z,acc_x,acc_y,acc_z, 
            roll, pitch, yaw,disturbance,control_type,midpoints):
    # Plot individual 2D plots for pos_x, pos_y, pos_z, roll, pitch, yaw
    fig, axs = plt.subplots(4, 3, figsize=(18, 10))
    fig.suptitle(f"2D State Space Plot for {control_type} Control on {disturbance} Terrain")
    ...
    plt.tight_layout()
    plt.savefig(fg.store_clean_data(disturbance,control_type) +"2d_state_space.png")
    plt.clf()
    plt.close()
...
def plot_3d_phase_space_pos(pos_x, pos_y, pos_z,disturbance,control_type,bottom_peaks,top_peaks):
    # Plot 3D phase space plot
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ...
    plt.clf()
    plt.close()
def plot_rotated_helix(pos_x, pos_y, pos_z,disturbance,control_type,bottom_peaks):
    # Plot 3D phase space plot
    bottom_peaks = np.array(bottom_peaks)
    bottom_peaks = bottom_peaks[bottom_peaks<min(len(pos_x),len(pos_y),len(pos_z))]

    fig, axs = plt.subplots(2, 2, figsize=(18, 10))
    fig.suptitle(f'Rotated Helix Plot for {control_type} Control on {disturbance} Terrain')
    ...
    plt.tight_layout()
    plt.savefig(fg.store_clean_data(disturbance,control_type)+"rotated_helix.png")
    plt.clf()
    plt.close()

def plot_original_helix(pos_x, pos_y, pos_z,disturbance,control_type,bottom_peaks):
    # Plot 3D phase space plot
    ...
    fig, axs = plt.subplots(2, 2, figsize=(18, 10))
    fig.suptitle(f'Original Helix Plot for {control_type} Control on {disturbance} Terrain')
    ...
    plt.tight_layout()
    plt.savefig(fg.store_clean_data(disturbance,control_type)+"original_helix.png")
    plt.clf()
    plt.close()
...
def main(disturbance,control_type):
    ...
    plot_2d(timestamps, pos_x, pos_y, pos_z, vel_x, vel_y, vel_z, 
            acc_x,acc_y,acc_z,roll, pitch, yaw, disturbance,control_type,midpoints)

    # Plot 3D phase space
    plot_3d_phase_space_pos(pos_x, pos_y, pos_z,disturbance,control_type,bottom_peaks,top_peaks)
    ...
    #plot 2d gait cycle
    plot_gait(plot_x,plot_y,plot_z,bottom_peaks, disturbance,control_type)

    #plot rotated data
    plot_rotated_helix(plot_x,plot_y,plot_z,disturbance,control_type,bottom_peaks)

    #plot original data
    plot_original_helix(pos_x, pos_y, pos_z,disturbance,control_type,bottom_peaks)